	...
	if event is InputEventMouseButton:
		...
		if event.keycode == KEY_Q and event.is_pressed() and !pressed:
			...
			while next_cell != target_cell:
				...
				tile = tilemap.get_cell_tile_data(layer, next_cell)
				...
				new_polygon.global_position = Vector2(next_pos.x, next_pos.y + y_sort_origin)
...
func select_tile(tilemap : TileMap) -> Dictionary:
	...
	if tile_data:
		...
		var next_cell = tilemap.local_to_map(target_pos)
		var next_pos := Vector2(target_pos.x, target_pos.y - 16)
		print_debug("selected %s, %s | %s" % [next_cell, next_pos, tilemap.get_layers_count()])
		var tmp : TileData
		...
		for l in range(target_layer + 1, tilemap.get_layers_count()):
			...
			# use next_pos since 'inside(...)' should always be true for tiles above the selected one
			tmp = try_get_tile(l, next_pos, Vector2i(next_cell.x, next_cell.y - (i * 2)), next_pos, tilemap)
